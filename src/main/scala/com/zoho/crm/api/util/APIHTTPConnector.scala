...
class APIHTTPConnector {
   ...
   */
  def setRequestMethod(httpMethod :String) :Unit = {this.requestMethod = httpMethod}
   ...
   */
  def setHeaders(headers :mutable.HashMap[String,String]) :Unit = {this.headers = headers}
   ...
   */
  def setParams(parameters :mutable.HashMap[String,String]) :Unit = {this.parameters = parameters}

  /**
   * This is a setter method to set the API request body.
   * @param requestBody A Object containing the API request body.
   */
  def setRequestBody(requestBody: Any): Unit = {this.requestBody = requestBody}
  ...
  private def getHttpClient:CloseableHttpClient = {
    ...
    if (requestProxy != null) {
      ...
      if (requestProxy.getUser.isDefined) {
        val credsProvider = new BasicCredentialsProvider
        credsProvider.setCredentials(new AuthScope(proxyHost, proxyPort), new NTCredentials(requestProxy.getUser.get, requestProxy.getPassword.get, null, requestProxy.getUserDomain.get))
  ...
  private def proxyLog(requestProxy: RequestProxy):String = {
    ...
    if (requestProxy.getUser.isDefined) proxyDetails += " , " + Constants.PROXY_USER + requestProxy.getUser.get
    if (requestProxy.getUserDomain.isDefined) proxyDetails += " , " + Constants.PROXY_DOMAIN + requestProxy.getUserDomain.get
    proxyDetails
  }
}